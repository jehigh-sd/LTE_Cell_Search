...
typedef struct {
    ...
    XCp_corr_pss_sss_uio_map maps[ MAX_UIO_MAPS ];
} XCp_corr_pss_sss_uio_info;

/***************** Variable Definitions **************************************/
static XCp_corr_pss_sss_uio_info uio_info;
...
static int uio_info_read_name(XCp_corr_pss_sss_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCp_corr_pss_sss_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCp_corr_pss_sss_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCp_corr_pss_sss_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCp_corr_pss_sss_Initialize(XCp_corr_pss_sss *InstancePtr, const char* InstanceName) {
	XCp_corr_pss_sss_uio_info *InfoPtr = &uio_info;
