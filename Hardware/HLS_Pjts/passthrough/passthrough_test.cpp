...
hls::stream<sample_pkt> Out_R1;
hls::stream<sample_pkt> Out_I1;
hls::stream<sample_pkt> Out_R2;
hls::stream<sample_pkt> Out_I2;
...
int main()
{
    ...
    for(int i=0; i<NUM_INPUT_SAMPLES; i+=SAMPLES_PER_CYCLE)
    {
        ...
        for(uint8_t j = 0; j < SAMPLES_PER_CYCLE; j++)
        {
            ...
            t_r1 = temp_rf1[j]/(SCALE_FACTOR*32);
            t_i1 = temp_if1[j]/(SCALE_FACTOR*32);

            t_r2 = temp_rf1[j]/(SCALE_FACTOR*32);
            t_i2 = temp_if1[j]/(SCALE_FACTOR*32);
